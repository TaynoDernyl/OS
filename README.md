...
## xD Архитектура Компилятора 

	Счетчики и трассировка:

	bool trace = false; (по дефолту отключена)

	int labels_count = 0; (счетчик меток)

	int data_adress_count = 0; (счетчик занятой памяти)

	int code_adress_count = 0;	(счетчик кода)

	int swag_size = 0; (размер команд в файле .swg)
	...
	**ИА**
	...
	Имя char*
	Разрядность метки char* (byte, word)
	Адрес хранения переменной uint_16
	Значение переменной uint_8, uint_16
    ...
    **ИОФ**
	...
	Двух этапная компиляция:

	Первый проход: (при введении команд)

	Корректировка адреса команд
	Корректировка адреса данных
	Занесение прерываний в массив comands_for_compile
	Занесение адресов меток в массив labels
	Занесение значения переменных в массив variable

	Второй проход: (при вводе compile либо выходе)

	Подставление значений меток
	Подставление значений переменных
	Запись в бинарный файл 16-ричных команд процессора
	При включенной трассировке выводить каждую операцию в консоль

	Прерывание для ошибок:
	Выведение в консоль что ввел пользователь
	На каком этапе, по какому адресу, какая команда выдала ошибку
